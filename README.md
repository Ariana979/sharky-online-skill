# sharky-online — multiplayer HTML games from Claude Code

A Claude Code skill: describe a game in one sentence (or hand over an
existing single-file HTML game) and Claude builds it, wires in real
platform multiplayer, lets you playtest locally, and — only after you say
so — publishes it to [sharky.gg](https://sharky.gg) under your own
account. Real rooms, invite links, a server-ordered sync bus (~400ms
round trip; local echo makes it feel instant).
...
- Claude Code in any form — desktop app, terminal CLI, or claude.ai web
...
claude.ai web: the sandbox filesystem is ephemeral — vendor this repo
into your project repository at `.claude/skills/sharky-online/` (copy or
submodule) so it checks out together with the project.

## Use

Just talk to Claude Code — any language works:
...
Claude hands you a local two-pane playtest link first; nothing goes live
until you say “publish”.

## What happens at publish

The first publish signs into YOUR sharky.gg account: Claude asks for the
account email (the one shown in your sharky.gg account settings — Apple
“Hide My Email” users: the relay address), the platform emails you a
6-digit code, you paste it back. Later publishes on the same machine are
zero-prompt. The skill ships zero secrets; games land in your account.
...
The web sandbox blocks outside domains by default. If the first publish
hits a network error, Claude prints the exact domains to allow — add
them under environment settings → Network access.
